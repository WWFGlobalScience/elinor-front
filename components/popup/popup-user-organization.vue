<template>
    <div class="popup__content popup--assessment-delete">
                ...
                <div class="input input--pr input--multiselect">
                    <div class="multiselect__wrap">
                        <multiselect
                            ...
                            <span slot="noResult" slot-scope="props" class="text-xxs text-grayy-lighter">{{ $t('default.noresults') }} ({{ props.search }}) <hr class="my-4"> <button @click="$event.preventDefault();$event.stopPropagation(); createOrganizationAndAssociate(props.search)" role="button" class="btn btn--sm btn--border-turqy mt-2">{{ $t('default.create') }}</button></span>
                        </multiselect>
        ...
        <div class="form__row flex justify-end mt-10">
            <button @click="popupState({active: false})" type="button" class="btn btn--opacity--child">
