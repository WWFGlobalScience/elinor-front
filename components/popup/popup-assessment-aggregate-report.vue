    ...
    <div class="popup__content">
        <section class="border-b pb-12 mb-10">
            <div class="elinor-card--question pt-0">
                ...
                <div class="elinor-card--question__info">
                    <h2
                        ...
                        v-html="
                            $t(
                                'pages.assessments.list.aggregateReport.subtitle',
                            )
                        "
                    />
                </div>
            ...
            <form class="form form--assessment grid grid-cols-4 gap-6">
                <div class="input input--multiselect col-span-3 md:col-span-4">
                    <label class="label">{{
                        $t(
                            'pages.assessments.list.aggregateReport.byManagementAreas',
                        )
                    }}</label>
                ...
                <div
                    ...
                    class="input input--multiselect col-span-1 md:col-span-4"
                >
                    <label class="label">{{
                        $t('pages.assessments.list.aggregateReport.byYear')
                    }}</label>
                    <div class="multiselect__wrap">
                        <multiselect
                            :value="filters.year"
                ...
                <div
                    ...
                    class="input input--multiselect col-span-2 md:col-span-4"
                >
                    <label class="label">{{
                        $t('pages.assessments.list.aggregateReport.byCountries')
                    }}</label>
                    <div class="multiselect__wrap">
                        <multiselect
                ...
                >
                    <label class="label">{{
                        $t('pages.assessments.list.aggregateReport.byType')
                    }}</label>
                    <div class="multiselect__wrap">
                        <multiselect
                            :value="filters.type"
                            :options="
                                types.map((type) => {
                                    return {
                                        id: type,
                                        name: $t(
                                            'pages.assessments.collectionMethods.' +
                                                type,
                                        ),
                                    };
                                })
        ...
        >
            <h3
                class="mb-10"
                v-html="
                    $t('pages.assessments.list.aggregateReport.filtersSelected')
                "
            />
            <ul
                class="filters-selected flex flex-row justify-items-start items-center flex-wrap gap-3"
            >
                <li
                    class="elinor__tag"
                ...
                >
                    ...
                    >
                        <img
                            src="~/assets/img/ico-close-popup.svg"
                            alt="Delete"
                        />
        ...
        <section class="border-b pb-12 mb-10" v-if="assessments.length">
            <h3
                ...
                "
            />
            <p class="w-[100%] text-right mb-4">
                <span class="text-turqy-darker font-bold">{{
                    assessments.length
                }}</span>
            ...
            <ul class="ui-rounded-border p-0">
                ...
                >
                    ...
                    >
                        ...
                        <div class="flex-grow">
                            <h4
                                class="font-bold text-turqy-darker text-[14px] mb-1"
                            >
                                {{ assessment.name }}
                            </h4>
                            <span class="text-[14px] text-grey"
                                >{{
                                    assessment.management_area.reported_size ||
                                    0
                                }}
                                {{
                                    $t(
                                        'pages.assessments.list.aggregateReport.coveringHA',
                                    )
                                }}</span
                            >
                        </div>
                        <div
                            class="flex-grow-0 flex flex-col align-center justify-center md:text-left text-center"
                        >
                            <span
                                class="text-[12px] text-grey mb-1"
                                v-html="
                                    $t(
                                        'pages.assessments.list.aggregateReport.year',
                                    )
                                "
                            />
                    ...
                    <div
                        class="flex-grow-0 border-l p-6 flex flex-col justify-center items-center"
                    >
            ...
            <div class="elinor-card--question pt-0">
                ...
                <div class="elinor-card--question__info">
                    <h2
                        ...
                        v-html="
                            $t(
                                'pages.assessments.list.aggregateReport.giveName',
                            )
                        "
                    />
                </div>
            ...
            <div class="name">
                <div class="input">
                    <input
                        ...
                        :placeholder="
                            $t(
                                'pages.assessments.list.aggregateReport.giveNamePlaceholder',
                            )
                        "
                        :value="name"
        ...
        <div class="report-wrap" v-if="assessments.length > 0">
            <template
                v-for="(item, index) in Math.ceil(assessments.length / 6)"
            >
                <report-pages-aggregate
                    ...
                    :assessments="
                        assessments.filter(
                            (assessment) =>
                                selectedAssessments.indexOf(assessment.id) !==
                                -1,
                        )
                    "
                    :name="name"
...
export default {
    ...
    computed: {
        ...mapState({
            ...
            assessments: (state) =>
                state.assessments.aggregateReport.assessments,
    ...
    methods: {
        ...mapActions({
            ...
            removeFilterAggregateReport:
                'assessments/removeFilterAggregateReport',
...
*/
            ...
            if (this.name === null || this.name === '') {
                this.errors.name.push(
                    this.$t(
                        'pages.assessments.list.aggregateReport.nameRequiredError',
                    ),
                );
            } else {
                if (this.name.length > 50) {
                    this.errors.name.push(
                        this.$t(
                            'pages.assessments.list.aggregateReport.nameLengthError',
                        ),
                    );
                }
            }
            ...
            if (i < Math.ceil(this.selectedAssessments.length / 6)) {
                doc.html(document.querySelector(`#aggregate-report-${i}`), {
                    callback: (doc) => {
                        ...
                        if (
                            i + 1 <
                            Math.ceil(this.selectedAssessments.length / 6)
                        ) {
