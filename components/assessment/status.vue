<template>
  <div class="is-finalized" v-if="isFinalized()">
    <!-- <template v-if="isPublished()">
            <img
                src="~/assets/img/ico-megaphone.svg"
                class="ico-circle"
                alt="Published"
            />
...
export default {
  ...
  methods: {
    isPublished() {
      return (
        this.assessment.data_policy === 90 && this.assessment.status === 10
      );
    },
    isFinalized() {
      return (
        this.assessment.data_policy === 10 && this.assessment.status === 10
      );
    },
  },
};
