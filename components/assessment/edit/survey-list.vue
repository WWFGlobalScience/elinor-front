        ...
        <div class="container">
            <div class="elinor__survey-list">
                <template v-for="attribute in attributes">
                    <div class="title-border" :id="`anchor-attribute-${attribute.id}`">
                        ...
                        <div class="title-aside">
                            <span v-if="attribute.required === true" class="att-wrap mandatori-att">
                                ...
                                <span>{{ $t("pages.assessments.edit.mandatoryAttribute") }}</span>
                            </span>
                            <span v-if="attribute.required === false && !isAttributeChecked(attribute)" class="att-wrap inactive-att">
                                ...
                                <span>{{ $t("pages.assessments.edit.inactiveAttribute") }}</span>
                            </span>
                            <span v-if="attribute.required === false && isAttributeChecked(attribute)" class="att-wrap active-att">
                                ...
                                <span>{{ $t("pages.assessments.edit.activeAttribute") }}</span>
                            </span>
                        </div>
                    </div>
                    ...
                    <div v-for="(question, index) in getAttributeQuestions(attribute)"
                        :class="[
                        ...
                        <div class="right">
                            <div v-if="isAttributeChecked(attribute)"
                                :class="{
                                ...
                                <template v-if="isAnswered(question)">
                                    ...
                                    <template v-if="getAnswerExplanation(question)" >
                                        <div class="txt txt--explanation">
                                            {{ $t("pages.assessments.edit.tabs.survey.questions.explanation") }}
                                        </div>
                                        <div class="answer" v-html="getAnswerExplanation(question)"></div>
                                    </template>
                                </template>
                                <div v-if="!isAnswered(question)" class="txt__uncomplete">
                                    {{ $t("pages.assessments.edit.tabs.survey.questions.notCompleted") }}
                                </div>
                                <nuxt-link :to="`/assessments/edit/${assessment.id}/the-survey/${question.id}`" class="btn btn--opacity--child btn--uncomplete">
                                    <span class="btn--opacity__target" v-if="!isAnswered(question)">
                                        {{ $t("pages.assessments.edit.tabs.survey.questions.answerQuestionButton") }}</span>
                                    <span class="btn--opacity__target" v-if="isAnswered(question)">
                                        {{ $t("pages.assessments.edit.tabs.survey.questions.editQuestionButton")}}
                                    </span>
