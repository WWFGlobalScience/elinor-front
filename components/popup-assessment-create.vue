            ...
            <div class="form__group">
                <div class="form__row">
                    <div class="input input--multiselect input--1-4">
                        <label class="label">{{ $t( 'pages.assessments.list.create.popup.labels.year' ) }}*</label>
                        <div class="multiselect__wrap">
                            <multiselect placeholder="" :value="selectYear.value" :options="selectYear.options" :multiple="false" :searchable="true" :showLabels="false" :allow-empty="false" open-direction="bottom" :hide-selected="false" @input="onSelectYearChange" >
                ...
                <div class="form__row">
                    <div class="input">
                        <label class="label">{{ $t( 'pages.assessments.list.create.popup.labels.name' ) }}*</label>
                        <input type="text" name="name" :placeholder="$t( 'pages.assessments.list.create.popup.placeholders.name' )" required>
                    </div>
                </div>
                <div class="form__row -mt-3">
                    <div class="input">
                        <div class="legend">{{ $t( 'pages.assessments.list.create.popup.nameCriteria' ) }}</div>
                    </div>
                </div>
                ...
                <div class="form__row flex justify-end mt-10">
                    <button type="submit" form="form--assessment-create" class="btn btn--opacity--child">
                        <span class="btn--opacity__target">{{ $t( 'pages.assessments.list.create.popup.submit' ) }}</span>
