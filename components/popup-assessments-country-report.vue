<template>
  <div class="popup__content popup--filters">
    <h4 class="c-title--modal mb-6">Select countries to generate report</h4>
    <form class="form form--ma-selector form--mt-0">
          ...
          <div class="input input--multiselect input--1-2">
            <div class="multiselect__wrap">
              <multiselect
                :placeholder="
                  $t(
                    'pages.assessments.list.country-report.popup.placeholders.countries',
                  )
                "
      ...
      <section class="mt-10">
        <div
          v-if="country !== null && report !== null && report.length === 0"
          class="mb-10 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative"
          role="alert"
        >
          <strong class="font-bold">{{
            $t(
              'pages.assessments.list.country-report.popup.countryWithoutAssessments',
            )
          }}</strong>
        </div>
        <button
          ...
          type="button"
          @click="onApply"
          class="btn--border-turqy btn--opacity--child btn--tab-disabled"
          style="float: left"
        >
          <span class="btn--opacity__target">{{
            $t('pages.assessments.list.country-report.popup.apply')
          }}</span>
