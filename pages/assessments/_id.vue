    ...
    <article class="page page--managed-area">
        <header class="header--page">
            <div class="container">
                ...
                <h2 class="ma__name">{{ assessment.name }}</h2>
            </div>
        </header>
    ...
    methods: {
        ...
        loadAssessment() {
            const AssessmentPromise = new Promise(( resolve, reject ) => {
                setTimeout(() => {
                    resolve( this.getFirstMa(), this.getAssessment( this.id ) )
                }, 300)
            })

            AssessmentPromise.then( this.mapCreate() )
        }
    },
    mounted() {
        this.loadAssessment()
