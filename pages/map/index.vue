...
export default {
    ...
    computed: {
        ...
        filteredData() {
            if (this.data) {
                var filtered = this.formatData().filter(
                    (f) =>
                        this.country
                            ? f.properties.management_area.countries &&
                              f.properties.management_area.countries.includes(
                                  this.country,
                              )
                            : true,
                ...
                if (this.attribute) {
                    ...
                    filtered.map(
                        (x) => (x.properties.filterScore = x.properties.score),
                    );
                }
                ...
                if (this.range) {
                    ...
                    filtered = filtered.filter(
                        (x) =>
                            Number(x.properties.score) >=
                                ranges[this.range][0] &&
                            Number(x.properties.score) < ranges[this.range][1],
                    );
                }
            ...
            return Array.from(
                new Set(
                    this.filteredData.reduce(
                        (carry, current) => [
                            ...carry,
                            ...current.properties.attributes.map(
                                (e) => e.attribute,
                            ),
                        ],
                        [],
    ...
    methods: {
        ...
        async onMapLoaded(e) {
            ...
            map.on('mouseenter', 'markersLayer', () => {
                map.getCanvas().style.cursor = 'pointer';
            });
            map.on('mouseleave', 'markersLayer', () => {
                map.getCanvas().style.cursor = 'default';
            });
        ...
        formatData() {
            ...
            this.data.forEach((feature) => {
                ...
                if (feature.geometry.type == 'Point') {
                    ...
                    polygon.properties.hectares = center.properties.hectares = (
                        turf.area(polygon) * 0.0001
                    ).toFixed(0);
        ...
        showDetail() {
            ...
            if (this.popup.assessment.geometry.type == 'MultiPolygon') {
                ...
                this.map.easeTo({
                    center: this.popup.assessment.geometry.coordinates,
                    zoom: 8,
                });
            }
        },
        closeDetail() {
            ...
            this.filter({
                name: 'country',
                value: this.popup.assessment.properties.management_area
                    .countries[0],
            });
