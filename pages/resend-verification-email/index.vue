  ...
  <article class="page page--flushed">
    <section class="section section--sign-in">
      ...
      <div class="elinor__card elinor__card--sign-in">
        ...
        <div class="elinor__card--sign-in__info">
          <div>
            <p class="text-xl mb-1.5">
              {{ $t('pages.auth.resendVerificationEmail.title') }}
            </p>
          </div>
        </div>

        <p class="text-base">
          {{ $t('pages.auth.resendVerificationEmail.subtitle') }}
        </p>
        <div
          v-if="error"
        ...
        <div
          v-if="alerts.resendVerificationEmailSent"
          class="bg-green-100 mt-10 border border-green-400 text-white-700 px-4 py-3 rounded relative"
          role="alert"
        >
          <strong class="font-bold">{{
            $t('pages.auth.forgotPassword.alerts.forgotPasswordEmailSent.title')
          }}</strong>
            ...
            <div class="form__row">
              <div class="input">
                <input
                  type="email"
                  :placeholder="
                    $t('pages.auth.resendVerificationEmail.emailInput')
                  "
                  v-model="email"
                  required
                />
            ...
            <div>
              ...
              >
                <span class="btn--opacity__target">{{
                  $t('pages.auth.resendVerificationEmail.sendEmailButton')
                }}</span>
              ...
              >
                <span class="btn--opacity__target">{{
                  $t('pages.auth.resendVerificationEmail.tryAgainButton')
                }}</span>
                <img src="~/assets/img/ico-signin-turqy.svg" />
              </button>
            </div>
