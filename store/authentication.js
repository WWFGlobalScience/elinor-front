...
export const actions = {
  async signIn(state, credentials) {
    ...
    try {
      ...
      if (error.response.data.non_field_errors) {
        if (
          error.response.data.non_field_errors[0].indexOf('email') !== -1 ||
  ...
  async signUp(state, form) {
    ...
    await this.$axios
      ...
      .catch((e) => {
        ...
        setTimeout(() => {
          this.$scrollTo(
            '#' + Object.keys(error).filter((e) => e !== 'non_field_errors')[0],
            { offset: -100 },
          );
        });
  ...
  async logout(state) {
    ...
    if ('serviceWorker' in navigator && 'caches' in window) {
      try {
        ...
        console.error(
          'Error clearing caches or unregistering service workers:',
          error,
        );
      }
    }
  ...
  async resetProfileUpdatedSuccessfully(state) {
    state.commit('setAlert', {
      name: 'profileUpdatedSuccessfully',
      value: false,
    });
